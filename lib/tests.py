...
class CaldavBandTest(CaldavTestBase):
    ...
    def test_calfeed_event_enddate(self):
        ...
        self.testgig.save()
        cf = make_band_calfeed(self.joeuser, self.band.gigs.all())
        self.assertTrue(cf.find(b'DTSTART:20200229T143000')>0)
    ...
    def test_calfeed_translation(self):
        ...
        self.testgig.setlist = 'test set'
        self.testgig.save()
        cf = make_band_calfeed(self.joeuser, self.band.gigs.all(),'de', self.joeuser.cal_feed_id)
...
# 
#     def test_calfeed_event(self):
#         cf = make_calfeed(self.joeuser, self.band.gigs.all())
#         self.assertTrue(cf.find(b'EVENT')>0)
# 
#     def test_calfeed_event_no_enddate(self):
#         cf = make_calfeed(self.joeuser, self.band.gigs.all())
