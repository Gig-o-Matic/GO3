...
class MemberEmailTest(TestCase):
    ...
    def test_markdown_mail(self):
        message = prepare_email(
            self.member.as_email_recipient(), 't:**Markdown**')
...
class InviteTest(TemplateTestCase):
    def setUp(self):
        ...
        Assoc.objects.create(member=self.band_admin, band=self.band,
                             status=AssocStatusChoices.CONFIRMED, is_admin=True)
    ...
    def test_invite_one(self):
        ...
        self.assertEqual(str(messages[0]),
                         'Invitations sent to new@example.com')
    ...
    def test_invite_member(self):
        ...
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]),
                         'Invitations sent to jane@example.com')
        self.assertEqual(Invite.objects.count(), 1)
    ...
    def test_invite_invalid(self):
        ...
        self.assertEqual(len(messages), 1)
        self.assertEqual(
            str(messages[0]), 'Invalid email addresses: notanemailaddress')
        self.assertEqual(Invite.objects.count(), 0)
    ...
    def test_invite_gets_language(self):
        ...
        self.client.post(reverse('member-invite', args=[self.band.id]),
                         {'emails': 'new@example.com'})
        self.assertEqual(Invite.objects.filter(
            email='new@example.com', language='de').count(), 1)
    ...
    def test_invite_delete_own(self):
        ...
        response = self.client.get(
            reverse('member-invite-delete', args=[invite.id]))
        self.assertRedirects(response, reverse(
            'member-detail', args=[self.janeuser.id]))
        self.assertEqual(Invite.objects.count(), 0)
    ...
    def test_invite_new_email(self):
        ...
        self.assertIn(reverse('member-invite-accept',
                              args=[invite.id]), message.body)
        self.assertNotIn(MISSING, message.body)

    @flag_missing_vars
    def test_invite_existing_email(self):
        invite = Invite.objects.create(
            email=self.janeuser.email, band=self.band)
        message = mail.outbox[0]
    ...
    def test_invite_no_band_email(self):
        ...
        message = mail.outbox[0]
        self.assertEqual(
            message.subject, 'Confirm your Email to Join Gig-o-Matic')
        self.assertIn(reverse('member-invite-accept',
                              args=[invite.id]), message.body)
        self.assertNotIn(MISSING, message.body)
    ...
    def test_accept_invite_logged_in(self):
        ...
        self.assertRedirects(response, reverse(
            'member-detail', args=[self.janeuser.id]))
        self.assertEqual(Assoc.objects.filter(
            band=self.band, member=self.janeuser).count(), 1)
        self.assertEqual(Invite.objects.count(), 0)
    ...
    def test_accept_invite_duplicate_assoc(self):
        ...
        response = self.client.get(
            reverse('member-invite-accept', args=[invite.id]), {'claim': 'true'})
        self.assertRedirects(response, reverse(
            'member-detail', args=[self.joeuser.id]))
        self.assertEqual(Assoc.objects.filter(
            band=self.band, member=self.joeuser).count(), 1)
        self.assertEqual(Invite.objects.count(), 0)
    ...
    def test_invite_sets_language(self):
        ...
        self.assertEqual(
            self.client.cookies[settings.LANGUAGE_COOKIE_NAME].value, 'de')
    ...
    def test_signup(self):
        ...
        self.assertEqual(Invite.objects.filter(
            email='new@example.com', band=None).count(), 1)
    ...
    def test_signup_duplicate(self):
        ...
        self.assertEqual(Invite.objects.filter(
            email='joe@example.com').count(), 0)

    def test_signup_invalid(self):
        response = self.client.post(
            reverse('member-signup'), {'email': 'invalid'})
        self.assertOK(response)
...
class MemberDeleteTest(TestCase):
    def setUp(self):
        ...
        Assoc.objects.create(member=self.band_admin, band=self.band,
                             is_admin=True, status=AssocStatusChoices.CONFIRMED, email_me=False)
    ...
    def create_gig(self, the_member, title="New Gig", start_date='auto', set_date='auto', end_date='auto'):
        thedate = timezone.datetime(2100, 1, 2, 12, tzinfo=pytz_timezone(
            'UTC')) if start_date == 'auto' else start_date
        return Gig.objects.create(
            title=title,
            band_id=self.band.id,
            date=thedate,
    ...
    def test_delete_member_email(self):
        ...
        self.joeuser.delete()
        self.assertEqual(self.joeuser.email,
                         'user_{0}@gig-o-matic.com'.format(self.joeuser.id))
    ...
    def test_delete_member_noplans(self):
        ...
        self.joeuser.delete()
        g2 = self.create_gig_form(
            contact=self.band_admin, user=self.band_admin)
    ...
    def test_login_delete(self):
        ...
        self.assertTrue(self.client.login(
            email=self.joeuser.email, password='ThisIsPass716'))
        self.joeuser.delete()
