...
names=[]
with urllib.request.urlopen('http://names.drycodes.com/100?nameOptions=boy_names&separator=space&format=json') as f:
    names=json.loads((f.read().decode('utf-8')))
with urllib.request.urlopen('http://names.drycodes.com/100?nameOptions=girl_names&separator=space&format=json') as f:
    names += json.loads((f.read().decode('utf-8')))
...
band_names=[]
b_prefix=['The ','']
with urllib.request.urlopen('http://names.drycodes.com/25?nameOptions=funnyWords&separator=space&format=json') as f:
    b_main= json.loads(f.read().decode('utf-8'))
b_type=[' Band',' Brass Band',' Barching Band','']

with urllib.request.urlopen('http://names.drycodes.com/25?nameOptions=cities&separator=space&format=json') as f:
